...
function MobileSkillTimeline({ items }) {
  return (
    <div className="grid gap-4 md:hidden">
      {items.map((item) => {
        ...
        return (
          <article
            key={item.id}
            className="rounded-[1.5rem] border border-white/10 bg-[#070b12]/88 p-5 shadow-[0_16px_60px_rgba(0,0,0,0.35)] backdrop-blur-xl"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex min-w-0 items-start gap-3">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.05] text-lg text-white">
                  <Icon />
                </div>
...
function RadialOrbitalTimeline({ timelineData: items }) {
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <MobileSkillTimeline items={items} />

      <div
        ref={containerRef}
        onClick={handleContainerClick}
        className="relative hidden h-[760px] overflow-hidden md:block"
      >
      ...
      <SectionReveal className="relative z-10 mb-12">
        <div className="space-y-5">
          ...
          <div className="space-y-4">
            <h2 className="max-w-4xl text-3xl font-black leading-[0.92] tracking-[-0.06em] text-white sm:text-4xl md:text-6xl">
              Technical{" "}
